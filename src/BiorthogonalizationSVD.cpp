...
Matrix::Matrix(const int m, const int n, const double data_[])
  ...
  for (int j = 0; j < num_rows_; j++)
    for (int i = 0; i < B.num_cols_; i++) {
      ...
      for (int k = 0; k < num_cols_; k++)
        R.data_[j * B.num_cols_ + i] +=
            data_[j * num_cols_ + k] * B.data_[k * B.num_cols_ + i];
    }
  return R;
}
  ...
  for (int j = 0; j < num_rows_; j++)
    for (int i = 0; i < num_cols_; i++)
      R.data_[j * num_cols_ + i] =
          data_[j * num_cols_ + i] + B.data_[j * num_cols_ + i];
  return R;
}
  ...
  while (sqrt(s) <= (epsilon * epsilon) * (N * N)) {
    for (int i = 0; i < num_cols_ - 1; i++) {
      for (int j = i + 1; j < num_cols_; j++) {
        ...
        U.data_[i * num_cols_ + i] * U.data_[i * num_cols_ + j]

                                     U.data_[i * num_cols_ + j] =
            cos * U.data_[i * num_cols_ + j] + sin * U.data_[i * num_cols_ + j];
      }
...
int main(int argc, char *argv[]) {
  double temp[] = {2, 1, 3,  5,  6, 7, -1, 0, 7, 1, 10, 12, 0,  -1, -1,
                   3, 5, 16, -3, 7, 4, 3,  2, 1, 1, 6,  4,  -3, 7,  3};
  Matrix(5, 6, temp);
  Matrix(2, 2), R__(2, 2);
